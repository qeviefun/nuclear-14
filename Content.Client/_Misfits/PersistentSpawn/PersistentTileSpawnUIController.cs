// #Misfits Add - UI controller for the Persistent Tile Spawn Window.
// Manages the tile list, search filtering, and wires up placement via
// PersistentTileSpawnHijack so tile placements go through our persistence system.
using System;
...
/// <summary>
/// Mirrors <see cref="Robust.Client.UserInterface.Controllers.Implementations.TileSpawningUIController"/>
/// but routes all tile placement through the persistent-tile network channel.
/// </summary>
public sealed class PersistentTileSpawnUIController : UIController
{
    [Dependency] private readonly IPlacementManager _placement = default!;
    [Dependency] private readonly IResourceCache _resources = default!;
    [Dependency] private readonly ITileDefinitionManager _tiles = default!;

    private PersistentTileSpawnWindow? _window;
    private PersistentTileSpawnHijack? _hijack;
    private readonly List<ITileDefinition> _shownTiles = new();
    private bool _clearingTileSelections;
    // #Misfits Add - erase mode flag, mirrors vanilla TileSpawningUIController
    ...
    private void EnsureWindow()
    {
        ...
        _window = UIManager.CreateWindow<PersistentTileSpawnWindow>();
        ...
        // #Misfits Add - wire erase button, mirrors vanilla TileSpawnWindow erase toggle
        ...
        // Create the placement hijack wired to our persistence system
        _hijack = new PersistentTileSpawnHijack(
            IoCManager.Resolve<IEntityManager>(),
            IoCManager.Resolve<IEntityNetworkManager>(),
            IoCManager.Resolve<IEntityManager>().System<SharedTransformSystem>(),
            _tiles);
    ...
    private void StartTilePlacement(int tileType)
    {
        ...
        _placement.BeginPlacing(newObjInfo, _hijack);
    }

    // #Misfits Add - erase toggle: placing tile ID 0 (space) signals server to erase the tile and remove its persistent record
    private void OnTileEraseToggled(ButtonToggledEventArgs args)
    {
        ...
        {
            _eraseTile = true;
            // Tile type 0 = space / empty tile; the hijack sends this to the server
            // which interprets it as a persistent erase (removes the record + sets tile to space).
    ...
    private void ClearTileSelection(object? sender, EventArgs e)
    {
        ...
        // #Misfits Add - also reset erase state when placement clears, mirrors vanilla
        _eraseTile = false;
