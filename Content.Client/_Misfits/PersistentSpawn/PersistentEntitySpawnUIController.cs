// #Misfits Add - UI controller for the Persistent Entity Spawn Window.
// Manages the prototype list, search filtering, and wires up placement via
// PersistentEntitySpawnHijack so spawns go through our persistence system.
using System;
...
/// <summary>
/// Mirrors <see cref="Robust.Client.UserInterface.Controllers.Implementations.EntitySpawningUIController"/>
/// but routes all spawn and erase actions through the persistent-entity network channel.
/// </summary>
public sealed class PersistentEntitySpawnUIController : UIController
{
    [Dependency] private readonly IConfigurationManager _cfg = default!;
    [Dependency] private readonly IPlacementManager _placement = default!;
    [Dependency] private readonly IPrototypeManager _prototypes = default!;

    private PersistentEntitySpawnWindow? _window;
    private PersistentEntitySpawnHijack? _hijack;
    ...
    private void EnsureWindow()
    {
        ...
        _window = UIManager.CreateWindow<PersistentEntitySpawnWindow>();
        ...
        _hijack = new PersistentEntitySpawnHijack(
    ...
    private void OnEntityEraseToggled(ButtonToggledEventArgs args)
    {
        ...
        _placement.Clear();
        if (args.Pressed)
            _placement.ToggleEraserHijacked(_hijack!);

        args.Button.Pressed = args.Pressed;
        _window.OverrideMenu.Disabled = args.Pressed;
    }

    private void OnEntityOverrideSelected(Robust.Client.UserInterface.Controls.OptionButton.ItemSelectedEventArgs args)
    {
        if (_window == null || _window.Disposed)
            return;
    ...
    // ── Entity list building (mirrors engine EntitySpawningUIController) ──────
    ...
    private void BuildEntityList(string? searchStr = null)
    {
        ...
        {
            var namesComparison = string.Compare(a.Name, b.Name, StringComparison.Ordinal);
    ...
    private void OnEntityButtonToggled(ButtonToggledEventArgs args)
    {
        ...
        // Use BeginPlacing with our hijack so the spawn goes through our persistence channel
        _placement.BeginPlacing(newObjInfo, _hijack);
