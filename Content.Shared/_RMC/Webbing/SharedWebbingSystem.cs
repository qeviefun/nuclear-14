// #Misfits Add - RMC webbing system ported from RMC-14 (MIT)
// Adapted: removed XenoComponent dependency
using System.Linq;
...
using static Content.Shared._RMC.Webbing.WebbingTransferComponent;
...
public abstract class SharedWebbingSystem : EntitySystem
{
    ...
    public bool Attach(Entity<WebbingClothingComponent> clothing, EntityUid webbing, EntityUid? user, out bool handled)
    {
        ...
        if ((whitelist is { } && !_whitelist.IsWhitelistPass(whitelist, webbing)) ||
            (whitelist is not { } && _tag.HasTag(webbing, ArmorWebbingTag)))
        {
            return false;
        }
