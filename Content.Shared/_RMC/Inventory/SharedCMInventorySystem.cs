// #Misfits Add - RMC inventory system ported from RMC-14 (MIT)
// Adapted: removed SkillsSystem, XenoComponent, DevouredComponent dependencies
using System.Diagnostics.CodeAnalysis;
using System.Linq;
using Content.Shared._RMC.Input;
...
public abstract class SharedCMInventorySystem : EntitySystem
{
    ...
    protected void OnHolsterEntInsertedIntoContainer(Entity<CMHolsterComponent> ent, ref EntInsertedIntoContainerMessage args)
    {
        ...
        if (ev.Unholsterable &&
            ...
            (ent.Comp.Whitelist is not { } whitelist ||
        ...
        bool emptyOnly = false)
    {
        ...
        // Sort slots so empty ones come first
        slots.Sort((a, b) =>
        {
            var aEmpty = a.ContainerSlot?.ContainedEntity == null;
            var bEmpty = b.ContainerSlot?.ContainedEntity == null;
            return bEmpty.CompareTo(aEmpty);
        });
    ...
    private bool Unholster(EntityUid user, SlotFlags flag, CMHolsterChoose choose, out bool stop)
    {
        ...
        while (enumerator.NextItem(out var item2))
        {
